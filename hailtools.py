...
class VariantMapping:
    ...
    def map_variant(self, mt, var_dict, snv=True):
        """
            ...
            for i, var in enumerate(tqdm(v)):
                ...
                filtered_mt = hl.filter_intervals(mt,
                                                  [hl.parse_locus_interval(x, reference_genome="GRCh38")
                                                   for x in [self.scan_interval(locus)]])
                ...
                if c:
                    ...
                    temp_mt.row.show()
                    aou_vars[k].append(var)
                    try:
                        aou_mt[k] = aou_mt[k].union_rows(temp_mt)
                    except:
                        aou_mt[k] = temp_mt
